...
namespace Vision.VisionPro
{
    public class VisionProService
    {
        ...
        public BllResult InitVision(CameraData cameraData)
        {
            ImageWidth = cameraData.Width; 
            ImageHeight = cameraData.Height;
        ...
        public BllResult CalibCheckerboard(Bitmap bitmap)
        {
            try
            {
                ImageConvertTool.InputImage = new CogImage8Grey(bitmap);
                ImageConvertTool.Run();
                CalibCheckerboardTool.InputImage = (CogImage8Grey)ImageConvertTool.OutputImage;// Vision.m_AcqFifoTool[3].OutputImage as CogImage8Grey;
                CalibCheckerboardTool.Calibration.ComputationMode = CogCalibFixComputationModeConstants.PerspectiveAndRadialWarp;
                CalibCheckerboardTool.Calibration.FeatureFinder = CogCalibCheckerboardFeatureFinderConstants.CheckerboardExhaustive;
                CalibCheckerboardTool.Calibration.FiducialMark = CogCalibCheckerboardFiducialConstants.None;
                CalibCheckerboardTool.Calibration.CalibrationImage = (CogImage8Grey)CalibCheckerboardTool.OutputImage;
                CalibCheckerboardTool.Calibration.Calibrate();
                CalibCheckerboardTool.Run();
                if(!CalibCheckerboardTool.Calibration.Calibrated)
                    return BllResultFactory.Error("校准失败");   
                return BllResultFactory.Sucess();       
            }
            catch (Exception ex)
            {
        ...
        public BllResult<Bitmap> GetCalibCheckerboardImage(Bitmap bitmap)
        {
            ...
            CalibCheckerboardTool.RunParams.SpaceToOutput = Cognex.VisionPro.CalibFix.CogCalibSpaceToOutputConstants.CalibratedSpace;
            CalibCheckerboardTool.Run();
            return BllResultFactory<Bitmap>.Sucess(CalibCheckerboardTool.OutputImage.ToBitmap());
        }
