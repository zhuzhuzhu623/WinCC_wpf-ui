...
namespace VisionProView
{
    public partial class DisplayView: UserControl
    {
        ...
        public DisplayView(CameraData data)
        {
            InitializeComponent();

            CameraData=data;

            Bitmap bitmap =new Bitmap("D:\\HalocnImage\\1.png");
            Display.Image = new CogImage8Grey(bitmap);
            ...
            //    if (result != null)
            //        Display.StartLiveDisplay(result.Operator, false);
            //}
            Display.MouseMode = Cognex.VisionPro.Display.CogDisplayMouseModeConstants.UserDefined;
            Display.PopupMenu = false;
            if (CameraData != null)
            {
                HorLine.StartX = 0;
                ...
                Display.InteractiveGraphics.Add(Circle, "C", true);
            }
        }
        ...
        private void MouseMoveForMotion(int pointX, int pointY)
        {
            var resultX = pointX;
            var resultY = pointY;
            int visionWidth = Display.Width;
            int visionHeight = Display.Height;
            int x = 0;
            int y = 0;
            int dirX = 0;
            int dirY = 0;
            ...
            {
                case 0:
                    Display.UserDefinedMouseCursor = ImageResources.SlowCursor[i];
                    break;
                case 1:
                    Display.UserDefinedMouseCursor = ImageResources.MediumCursor[i];
                    break;
                case 2:
                    Display.UserDefinedMouseCursor = ImageResources.FastCursor[i];
                    break;
            }
